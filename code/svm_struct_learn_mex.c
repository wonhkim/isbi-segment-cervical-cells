...
mexFunction (int nout, mxArray ** out, int nin, mxArray const ** in)
{
  ...
  mxArray const * prob_maps_array;
  mxArray const * ucms_array;
  mxArray const * nuclei_array;
  mxArray const * kernelFn_array ;
  int numExamples, ei ;
  mxArray * model_array;

  mexPrintf("svm_struct_learn_mex.c: mexFunction line 79\n") ;
  ...
  /* Parse SPARM ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ */
  /*    start reading the second input argument (data, loss, feature, constraint) */
  ...
  prob_maps_array = mxGetField(sparm_array, 0, "prob_maps");
  if (! prob_maps_array || ! mxIsCell(prob_maps_array) ||
      ! mxGetNumberOfElements(prob_maps_array) == numExamples)
      mexErrMsgTxt("SPARM.PROB_MAPS must be a cell array");
  
  // parm.ucms = ucms;
  ucms_array = mxGetField(sparm_array, 0, "ucms");
  if (! ucms_array || ! mxIsCell(ucms_array) ||
          ! mxGetNumberOfElements(ucms_array) == numExamples)
      mexErrMsgTxt("SPARM.UCMS must be a cell array");
  
  // parm.nuclei = nuclei;
  nuclei_array = mxGetField(sparm_array, 0, "nuclei");
  if (! nuclei_array || ! mxIsCell(nuclei_array) ||
          ! mxGetNumberOfElements(nuclei_array) == numExamples)
      mexErrMsgTxt("SPARM.NUCLEI must be a cell array");
  /* ------------------------------------------------------------ */
  ...
  for (ei = 0 ; ei < numExamples ; ++ ei) {
    ...
    sample.examples[ei].prob_map.mex = mxGetCell(prob_maps_array, ei);
    sample.examples[ei].ucm.mex = mxGetCell(ucms_array, ei);
    sample.examples[ei].nuclei.mex = mxGetCell(nuclei_array, ei);
  }
