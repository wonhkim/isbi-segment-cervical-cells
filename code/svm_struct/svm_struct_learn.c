		      ...
		      STRUCTMODEL *sm, int alg_type)
{
  ...
  if(USE_FYCACHE) {
    ...
    for(i=0;i<n;i++) {
      fy=psi(ex[i].x,ex[i].y,ex[i].prob_map,ex[i].ucm,ex[i].nuclei,sm,sparm);/********************************/
      if(kparm->kernel_type == LINEAR) {
	  ...
	  if((!use_shrinking) || (opti[i] != opti_round)) {
	    ...
	    if(sparm->loss_type == SLACK_RESCALING) 
	      ybar=find_most_violated_constraint_slackrescaling(ex[i].x,
								ex[i].y,ex[i].prob_map,ex[i].ucm,ex[i].nuclei,
								sm,sparm);
	    else
	    ...
	    if(fycache) 
	      ...
	      fy=psi(ex[i].x,ex[i].y,ex[i].prob_map,ex[i].ucm,ex[i].nuclei,sm,sparm);/*****************************/
	    fybar=psi(ex[i].x,ybar,ex[i].prob_map,ex[i].ucm,ex[i].nuclei,sm,sparm);/********************************/
	...
	to the most violated constraint for example ex */
{
  ...
  (*argmax_count)++;
  if(sparm->loss_type == SLACK_RESCALING) 
    ybar=find_most_violated_constraint_slackrescaling(ex->x,ex->y,
                                                      ex->prob_map,
                                                      ex->ucm,ex->nuclei,
                                                      sm,sparm);
  else
    ybar=find_most_violated_constraint_marginrescaling(ex->x,ex->y,
                                                       ex->prob_map,
                                                       ex->ucm,ex->nuclei,
                                                       sm,sparm);
  ...
  if(fycached)
    ...
    fy=psi(ex->x,ex->y,ex->prob_map,ex->ucm,ex->nuclei,sm,sparm);
  fybar=psi(ex->x,ybar,ex->prob_map,ex->ucm,ex->nuclei,sm,sparm);
