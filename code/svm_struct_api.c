...
find_most_violated_constraint_slackrescaling (PATTERN x, LABEL y,
                                              PATTERN prob_map, PATTERN ucm, PATTERN nuclei,
                                              STRUCTMODEL *sm,
                                              STRUCT_LEARN_PARM *sparm)
{
  LABEL ybar ;
  ...
  mexSetTrapFlag (1) ;
  status = mexCallMATLAB(1, &ybar.mex, /*5*/8, args, "feval") ;
  mexSetTrapFlag (0) ;
...
// prob_map, ucm, nuclei
SVECTOR *
psi (PATTERN x, LABEL y, PATTERN prob_map, PATTERN ucm, PATTERN nuclei,
        STRUCTMODEL *sm, STRUCT_LEARN_PARM *sparm)     
{
  ...
  if (sm -> svm_model -> kernel_parm .kernel_type == LINEAR) {
    ...
    args[5] = ucm.mex;
    args[6] = nuclei.mex;
    status = mexCallMATLAB(1, &out, 7/*4*/, args, "feval") ;
